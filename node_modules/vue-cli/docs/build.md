...
```js
module.exports = {
  proxy: 'http://localhost:8181/api'
}
```

This way, when you fetch `/api/todos` in your Vue app, the development server will proxy your request to `http://localhost:8181/api/todos`.
...
module.exports = {
  proxy: {
    '/api/foo': 'http://localhost:8181/api',
    '/api/fake-data': {
